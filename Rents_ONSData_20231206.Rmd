...
Rentals_LAs_geog <- Rentals_LAsEng_Data %>% 
  left_join(LAs_boundaries, by=c("Area_Code"="Area_Code")) %>%
  ...
  mutate(Rentals_Cat = cut(Lower_quartile_num, breaks = c(1000, 1250, 1350, 1650, 3000),
                             ...
                             labels = c(" 1000 - 1249",
                                        ...
                                        " 1350 - 1649",
                                        " 1650 - 3000"))) %>%
#  filter(!is.na(Rentals_Cat)) %>%
#    filter(!is.na(BNG_E)) %>%
  data.frame()

#quantile(Rentals_LAs_geog$Lower_quartile_num)
...
Rentals_South_LAs_map <- Rentals_LAs_geog %>%
  ...
  labs(title= "Lower quartile monthly rent for 2 bedroom flat, London, 2022", 
       caption = paste0("Source: ONS, Chart: GLA demography")) +
#  labs(fill = "Monthly rent (£)") +
  guides(alpha="none") +
  coord_sf(xlim = c(480000,580000), ylim = c(130000, 220000)) 
#  annotate(geom = "rect", ymax = 220000, ymin = 130000, xmax = 580000, xmin = 480000, colour = "black", fill = NA) 

Rentals_South_LAs_map

# # Interactive map using ggplotly
# Rentals_South_LAs_map_int <- ggplotly(Rentals_South_LAs_map, tooltip = "text") %>%
...
# Rentals_South_LAs_map_int

# get error message: 
# Error in (function (..., deparse.level = 1)  : 
#  number of columns of matrices must match (see arg 8)
...
Rents_regions_LQ_line <- Rentals_Regions_Data %>%
  ...
  mutate(Region_factor = as.factor(Region)) %>%
  ggplot() +
  geom_line(aes(x = Date, y = Lower_quartile, group = Region_factor, color = Region_factor,
             text = paste("Date: ", Date_factor,
                          "<br>Region: ", Region_factor,
                          "<br>Monthly 2 bed rent: ", round(Lower_quartile, digits = 2)) 
  )) +
  theme_gla() +
  scale_x_discrete(breaks=c("2011_April", "2013_April", "2015_April","2017_April","2019_April", "2021_April", "2022_Oct")) +
   scale_color_manual(name='Region:', values = colour_palette) +
...
Rentals_Boroughs_change2019_map <- Rentals_Boroughs_change2019_geog_ed %>%
  ...
  theme(axis.title.x = element_blank(),
        ...
        axis.ticks.y = element_blank())+
  scale_fill_manual(values = catgreen4) + 
  labs(title= "<b>Change in lower quartile monthly rent for 2 bedroom flat, London, 2019-2022<b>", 
       caption = paste0("Source: ONS, Chart: GLA demography")) +
  labs(fill = "% change")
